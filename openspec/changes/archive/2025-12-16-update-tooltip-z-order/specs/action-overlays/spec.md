...
#### Scenario: Z-order below host metadata tooltip

- **WHEN** an action tooltip is displayed above a node
- **AND** the user hovers over that node to view host metadata
- **THEN** the host metadata tooltip renders above the action tooltip
- **AND** the host metadata tooltip is fully readable without obstruction
